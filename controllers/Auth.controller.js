...
const register = async (req, res) => {
    try {
        ...
        if (!reg.test(password))
            throw new Error('La contraseña no tiene formato correcto')
...
const verifyRegister = async (req, res) => {
    try {
        ...
        res.status(200).json({
            message: 'Se Verifico correctamente el usuario',
        })
    } catch (error) {
        res.status(400).json(error.message)
    }
...
// FUNCION PARA RECUPERAR LA CONTRASEÑA. DESDE ACA SE ENVIA EL CORREO ELECTRONICO
