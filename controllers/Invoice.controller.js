...
async function getInvoice(req, res) {
    try {
        ...
        if (!debts) {
            return res
                .status(404)
                .json({ message: 'Error al buscar los datos' })
        }
        let invoices = {}
        let phone = ''
        ...
        for (let i in debts) {
            ...
            var vto =
                today > new Date(debts[i].VTO1) ? debts[i].VTO2 : debts[i].VTO1
            var total =
                today > new Date(debts[i].VTO1)
                    ? debts[i].TOTAL2
                    : debts[i].TOTAL1
            var pdf =
            ...
            var voucher = `${typeInvoice[debts[i].COD_COM] || 'CSB'}-${debts[i][
                'SUC_COM'
            ]
                .toString()
                .padStart(4, '0')}-${debts[i]['NUM_COM']
                .toString()
                .padStart(8, '0')}`
            ...
            if (debts[i].NUMERO) {
                for (let j in invoices[debts[i].COD_SUM].list) {
                    if (
                        invoices[debts[i].COD_SUM].list[j].number &&
                        invoices[debts[i].COD_SUM].list[j].number ===
                            debts[i].NUMERO
                    ) {
                        invoiceExists = true
                        invoices[debts[i].COD_SUM].list[j].type =
                            debts[i].TIPO === 'EN'
                                ? `EN-${
                                      invoices[debts[i].COD_SUM].list[j].type
                                  }`
