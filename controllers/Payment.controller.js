...
const payLink = async (req, res) => {
  try {
    ...
    if (data.bills.length === 0)
      return res
        .status(400)
        .json({ status: 0, data: "No se seleccionaron facturas para pagar" });
    let result;
    const pay = {
      id_user: req.user.id,
...
const paymentMercadoPago = async (req) => {
  try {
    ...
    bills.forEach((bill) => {
      ...
      if (periodValue < oldPeriod.split("/").reverse().join(""))
        oldPeriod = bill.period;
      if (periodValue > newPeriod.split("/").reverse().join(""))
        newPeriod = bill.period;
    });
...
const paymentPayFun = async (req) => {
  try {
    ...
    const data = {
      ...
      name:
        user.name_register.toUpperCase() +
        " " +
        user.last_name_register.toUpperCase(),
...
const webhookResponse = async (req, res) => {
  ...
  const secret =
    "ba33249d2d35765461945b23b56afee086bcdd90ab516dd9229fc761712d40df";
...
const payCancelMp = async (req, res) => {
  try {
    ...
    const client = new MercadoPagoConfig({
      accessToken:
        "APP_USR-6180028893273834-012912-e4718b99fcaffd255c9b825a6ab3cfe5-1649301439",
    });
    const payment = new Payment(client);
    const paymentData = await payment.search(dataId)
