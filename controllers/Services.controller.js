...
async function getServicesCustomer(req, res) {
	try {
		...
		const groupedServices = services.reduce((acc, service) => {
			const code = dbCodes.find((code) => code.COD_SER === service.COD_SER)
			if (code) {
				...
				acc[key].services.push({
					...
					address: `${service.CALLECUENTA} ${parseInt(service.ALTURACALLECUENTA)}`,
					account: service.COD_SUM,
					detail: service['NUM_MED/NUMTEL'] !== '' ? (code.COD_SER === 9 ? 'TEL ' : 'MEDIDOR Nº ') + service['NUM_MED/NUMTEL'] : '',
		...
		if (medition) {
			filteredServices = filteredServices
				.map((serviceGroup) => ({
					...serviceGroup,
					services: serviceGroup.services.filter((service) => service.graphic),
				}))
...
async function getAllConsumptionsGroupedByAccount(req, res) {
	try {
		...
		const filteredServices = services.filter((service) => {
			const code = dbCodes.find((code) => code.COD_SER === service.COD_SER && code.SERVICIO_MEDIDO && service.BAJA_ADM === null)
			if (code) {
				return service
			}
		})
		...
		for (const service of filteredServices) {
			const consumptions = await consumoCustomer(service.COD_SER, service.COD_SUM)
			...
			groupedServices[service.COD_SER].accounts[service.COD_SUM] = {
				...
				unidad: dbCodes.find((code) => code.COD_SER === service.COD_SER).UNIDAD_MEDIDA,
			}
		}
