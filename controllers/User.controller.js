...
async function migrationUser(req, res) {
    try {
        ...
        for (const user in users) {
            const usersProcoop = await Persona_x_COD_SOC(
                users[user].number_customer || 0
            )
            userMigrate.push({
                name_register: users[user].first_name,
                last_name_register: users[user].last_name,
                email: users[user].email,
                email_verified: users[user].date_input
                    ? new Date(users[user].date_input)
                    : '',
            ...
            if (usersProcoop.length > 0) {
                ...
                User_procoopmembers.push({
                    id_user: userMigrate.length,
                    id_procoopmembers: procoopmembers.length,
            ...
            person_physical.push({
                ...
                born_date: users[user].birthday
                    ? new Date(users[user].birthday)
                    : '',
        ...
        const data = {
            ...
            procoopmembers: procoopmembers,
            User_procoopmembers: User_procoopmembers,
        }
