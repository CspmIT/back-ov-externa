...
const debtsCustomer = async (accounts, all = false) => {
	try {
		const query = `SELECT  dd.ID_FAC, dd.COD_COM,  dd.SUC_COM, fa.pagado, dd.NUM_COM, dd.TIPO, dd.FECHA, dd.COD_SOC, dd.COD_PER, dd.COD_SUM,
                  ...
                  WHERE dd.COD_SUM IN (:accounts) AND dd.FECHA  >= Dateadd(mm,-13,Getdate()) ${all ? '' : 'AND dd.SALDO != 0'} 
                  ORDER BY FECHA DESC`
		const result = await SequelizeOncativo.query(query, {
	...
	return db.sequelize.transaction(async (t) => {
		try {
			...
			if (!PersonProcoop) {
				...
				const [PersonProcoopControl, createdPerson] = await db.Person.findOrCreate({
				...
				if (dataProcoop.TIP_PERSO === 1) {
					const dataPersonPhysicalProcoop = {
						...
						born_date: dataProcoop.FEC_NAC ? new Date(`${dataProcoop.FEC_NAC} `) : null,
					...
					const [dataPersonPhysical, createdPhysical] = await db.Person_physical.findOrCreate({
